...
const FlatlyBookings = () => {
  ...
  const loadFlatBookingsPage = async (page: number) => {
    try {
      ...
      // Get all flat bookings and implement client-side pagination
      const allFlatBookings = await bookingService.getFlatBookings();
      
      const startIndex = page * PAGE_SIZE;
      const endIndex = startIndex + PAGE_SIZE;
      const pageData = allFlatBookings.slice(startIndex, endIndex + 1);
      
      setFlatBookings(pageData.slice(0, PAGE_SIZE));
